    ...
    public class CreateProductFunctionalTest {
        ...
        void testCreateProduct_Failure(ChromeDriver driver) {
            ...
            nameInput.sendKeys("");
            quantityInput.sendKeys("-11");
            submitButton.click();

            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
            WebElement quantityErrorMessage = wait.until(
                    ExpectedConditions.visibilityOfElementLocated(By.id("quantityInput-error"))
            );
            assertTrue(quantityErrorMessage.getText().contains("[Minimal diisi satu]"));

            WebElement nameErrorMessage = wait.until(
                    ExpectedConditions.visibilityOfElementLocated(By.id("nameInput-error"))
            );

            assertTrue(nameErrorMessage.getText().contains("[Nama produk harus ada]"));
        }
    }